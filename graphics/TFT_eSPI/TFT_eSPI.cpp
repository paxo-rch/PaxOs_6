...
void drawRoundRectWithBorder(int32_t x, int32_t y, int32_t w, int32_t h, int32_t radius, uint8_t borderSize, uint32_t color, uint32_t borderColor)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
  ...
  for(int i = 0; i < borderSize; i++)
    drawRoundRect(x + i, y + r + i, w - i*2, h - r - r - i*2, radius, borderColor);
